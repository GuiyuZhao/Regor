...
def farthest_point_sample(xyz, npoint):
    ...
    centroids = torch.zeros(npoint, dtype=torch.int64).cuda()  # 8*512
    '''构建一个距离矩阵表，用来存放点之间的最小距离值'''
    distance = torch.ones(N).cuda() * 1e10  # 8*1024
    '''batch里每个样本随机初始化一个最远点的索引（每个batch里从1024个点中取一个）'''
    farthest = torch.randint(0, N, (1,))  # type为tensor(8,)
    '''构建一个索引tensor'''
    for i in range(npoint):
        centroids[i] = farthest  # 第一个采样点选随机初始化的索引
        centroid = xyz[farthest, :]  # 得到当前采样点的坐标 B*3
        ...
        farthest = torch.argmax(distance)  # max函数返回值为value,index，因此取[1]值，即索引值，返回最远点索引
...
class Matcher():
    ...
    def cal_seed_trans(self, seeds, SC2_measure, src_keypts, tgt_keypts):
        ...
        # topk_num = min(seedwise_fitness.shape[1], 10)
        # _, batch_bests = seedwise_fitness.topk(dim=1, k=topk_num)
        # corr_idx_topk = knn_idx.gather(dim=2, index=knn_idx_fine1)
        # corr_idx_topk_topkseed = corr_idx_topk[:, batch_bests[0], :].view(-1)
        # corr_idx_topk_topkseed_unique = torch.unique(corr_idx_topk_topkseed)
        # src_keypts_topk = src_keypts[:, corr_idx_topk_topkseed_unique, :]
        # tgt_keypts_topk = tgt_keypts[:, corr_idx_topk_topkseed_unique, :]
    ...
    def post_refinement(self, initial_trans, src_keypts, tgt_keypts, it_num, weights=None):
        ...
        if self.Debug:
            print("pose_refinement之后的对应关系11")
            src = open3d.geometry.PointCloud()
            src.points = open3d.utility.Vector3dVector(src_keypts.view(-1, 3).cpu().numpy())
            tgt = open3d.geometry.PointCloud()
            tgt.points = open3d.utility.Vector3dVector(tgt_keypts.view(-1, 3).cpu().numpy())
            pred_inlier = (L2_dis < 0.1)[0]
            src_corr = open3d.geometry.PointCloud()
            src_corr.points = open3d.utility.Vector3dVector(
                src_keypts[:, pred_inlier, :].view(-1, 3).cpu().numpy())
            tgt_corr = open3d.geometry.PointCloud()
            tgt_corr.points = open3d.utility.Vector3dVector(
                tgt_keypts[:, pred_inlier, :].view(-1, 3).cpu().numpy())
            # draw_registration_corr2_vis(self.src_pcd, self.tgt_pcd, src_corr, tgt_corr,
            #                             np.linalg.inv(self.gt_trans[0].cpu().numpy()), self.src_color, self.tgt_color)
            visualization.draw_registration_corr2(src, tgt, src_corr, tgt_corr, np.linalg.inv(self.gt_trans[0].cpu().numpy()), 0.1)
            visualization.draw_registration_corr2(src, tgt, src_corr, tgt_corr, np.linalg.inv(self.gt_trans[0].cpu().numpy()), 0.5)

        refined_src_keypts = src_keypts[:, pred_inlier, :]
        refined_tgt_keypts = tgt_keypts[:, pred_inlier, :]
        return initial_trans, refined_src_keypts, refined_tgt_keypts
    ...
    def match_pair(self, src_keypts, tgt_keypts, src_features, tgt_features):
        ...
        # print(src_desc[0].shape)
        # print(src_desc[0][:, None, :].shape, tgt_desc[0][None, :, :].shape)
        # distance = torch.norm(src_desc[0][:, None, :].expand(-1, tgt_desc.shape[1], -1) - tgt_desc[0][None, :, :].expand(src_desc.shape[1], -1, -1)+1e-6, dim=2)
        ...
        src_desc_corr = src_desc[:, corr[:, 0]]
        tgt_desc_corr = tgt_desc[:, corr[:, 1]]
        ...
        #     src_key.points = open3d.utility.Vector3dVector(src_keypts_corr.view(-1, 3).cpu().numpy())
        #     tgt_key = open3d.geometry.PointCloud()
        #     tgt_key.points = open3d.utility.Vector3dVector(tgt_keypts_corr.view(-1, 3).cpu().numpy())
        #     visualization.draw_registration_corr2(src, tgt, src_key, tgt_key, np.linalg.inv(self.gt_trans[0].cpu().numpy()))


        return src_keypts_corr, tgt_keypts_corr, src_desc_corr, tgt_desc_corr
    ...
    def SC2_PCR(self, src_keypts, tgt_keypts):
        ...
        #################################
        final_trans, src_keypts, tgt_keypts = self.post_refinement(final_trans, src_keypts, tgt_keypts, 20)

        return final_trans, src_keypts, tgt_keypts

    def SM_select(self, src_keypts, tgt_keypts):
        """
        Input:
            - src_keypts: [bs, num_corr, 3]
            - tgt_keypts: [bs, num_corr, 3]
        ...
        #################################
        if num_corr > 100:
            src_keypts = src_keypts[:, :100, :]
            tgt_keypts = tgt_keypts[:, :100, :]
        ...
        #################################
        confidence = self.cal_leading_eigenvector(SC_measure, method='power')
        seeds = self.pick_seeds(src_dist, confidence, R=self.nms_radius, max_num=10)   # 第重叠的时候 nms_radius应该调小
    ...
    def estimator(self, src_keypts, tgt_keypts, src_features, tgt_features, gt_trans,  use_sampling=True):
        ...
        #################################
        src_keypts_corr, tgt_keypts_corr, src_desc_corr, tgt_desc_corr = self.match_pair(
            src_keypts,
            tgt_keypts,
            src_features,
            tgt_features)
        ...
        #################################
        pred_trans, src_keypts1, tgt_keypts1 = self.SC2_PCR(src_keypts_corr, tgt_keypts_corr)
        ...
        src_keypts_corr_final = src_keypts_corr[:, pred_labels[0], :]
        tgt_keypts_corr_final = tgt_keypts_corr[:, pred_labels[0], :]
        src_desc_corr_final = src_desc_corr[:, pred_labels[0], :]
        tgt_desc_corr_final = tgt_desc_corr[:, pred_labels[0], :]

        src_keypts_trans = transform(src_keypts, pred_trans)
        distance = torch.norm((src_keypts_trans[:, :, None, :] - tgt_keypts[:, None, :, :]), dim=-1)
        max_mask = distance <= distance.min(dim=-1)[0][:, :, None]
        distance_true_mask = distance<0.1
        distance_true_mask = distance_true_mask & max_mask
        bs_idx, src_idx, tgt_idx = torch.where(distance_true_mask)
        src_keypts_corr_final = src_keypts[:, src_idx, :]
        tgt_keypts_corr_final = tgt_keypts[:, tgt_idx, :]
        src_desc_corr_final = src_features[:, src_idx, :]
        tgt_desc_corr_final = tgt_features[:, tgt_idx, :]
        ...
        # src_desc_corr_final = src_desc_corr_final[:, sel_ind, :]
        # tgt_desc_corr_final = tgt_desc_corr_final[:, sel_ind, :]

        # src_keypts_corr_final, tgt_keypts_corr_final = self.SM_select(src_keypts_corr_final, tgt_keypts_corr_final)

        return pred_trans, src_keypts_corr_final, tgt_keypts_corr_final, src_keypts_corr, tgt_keypts_corr, src_desc_corr_final, tgt_desc_corr_final
